...
class Bst7Test extends FlatSpec with Matchers {
  val bst4 = Bst7(4)
  ...
  it should "insert less - char" in {
    Bst7('d').insert('a').left.get.value should equal('a')
  }
  ...
  it should "handle complex tree - sort out of order list" in {
    val bst = Bst7.fromList(List(4, 2, 6, 1, 3, 7, 5))
    Bst7.toList(bst) should equal((1 to 7).toList)
  ...
  it should "iterating one element" in {
    Bst7.toList(bst4) should equal(List(4))
  }

  it should "iterating over smaller element" in {
    Bst7.toList(Bst7.fromList(List(4, 2))) should equal(List(2, 4))
  }

  it should "iterating over larger element" in {
    Bst7.toList(Bst7.fromList(List(4, 5))) should equal(List(4, 5))
  }

  it should "iterating over complex tree" in {
    Bst7.toList(Bst7.fromList(List(4, 2, 1, 3, 6, 7, 5))) should equal((1 to 7).toList)
  }

  it should "iterating over complex tree - chars" in {
    Bst7.toList(Bst7.fromList(List('d', 'b', 'a', 'c', 'f', 'g', 'e'))) should
      equal(('a' to 'g').toList)
  }
}
