...
class Strain5Test extends FlatSpec with Matchers {
  ...
  it should "keep sequences" in {
    ...
    val result = Strain5.keep[Seq[Int]](sequences, seq => seq.contains(5))
    result should be(expected)
  }

  it should "handle empty discard" in {
    val result = Strain5.discard[Int](List(), x => x < 10)
    result should be(List.empty)
  }

  it should "discard first and last" in {
    val result = Strain5.discard[Int](List(1, 2, 3), x => x % 2 != 0)
    result should be(List(2))
  }

  it should "discard neither first or last" in {
    val result = Strain5.discard[Int](List(1, 2, 3, 4, 5), x => x % 2 == 0)
    result should be(List(1, 3, 5))
  }

  it should "discard strings" in {
    val words = List("apple", "zebra", "banana", "zombies", "cherimoya", "zelot")
    val result = Strain5.discard[String](words, s => s.startsWith("z"))
  ...
  it should "discard sequences" in {
    ...
    val result = Strain5.discard[Seq[Int]](sequences, seq => seq.contains(5))
    result should be(expected)
  }
}
