...
class SimpleLinkedList5Test extends FlatSpec with Matchers with GeneratorDrivenPropertyChecks {

  private implicit def arbitrarySimpleLinkedList5[T](implicit arbitraryTs: Arbitrary[Array[T]]): Arbitrary[SimpleLinkedList5[T]] =
    Arbitrary {
      arbitraryTs.arbitrary map (SimpleLinkedList5(_: _*))
    }

  it should "handle single item list" in {
    val list = SimpleLinkedList5().add(1)
  ...
  it should "handle two item list" in {
    val list = SimpleLinkedList5().add(1).add(2)
    list.value should be(1)
  ...
  it should "allow creation from a Seq" in {
    val list = SimpleLinkedList5.fromSeq(List(3, 2, 1))
  ...
  it should "handle reverse" in {
    val list = SimpleLinkedList5.fromSeq(List(1, 2, 3, 4, 5, 6))
  ...
  it should "return original arbitrary list from added list elements" in {
    forAll { xs: Seq[Int] =>
      val list = xs.foldLeft(SimpleLinkedList5[Int]())(_.add(_))
  ...
  it should "handle arbitrary generics" in {
    forAll { xs: Seq[String] =>
      assert(SimpleLinkedList5.fromSeq(xs).toSeq == xs)
    }
  }
}
