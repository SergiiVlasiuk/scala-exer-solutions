...
/** @version 1.3.0 */
class CustomSet5Test extends FunSuite with Matchers {

  // Empty test cases - Returns true if the set contains no elements
  test("sets with no elements are empty") {
    val set = CustomSet5.fromList(List())
    CustomSet5.empty(set) should be(true)
  }

  test("sets with elements are not empty") {
    val set = CustomSet5.fromList(List(1))
    CustomSet5.empty(set) should be(false)
  }

  // Contains test cases - Sets can report if they contain an element
  test("nothing is contained in an empty set") {
    val set = CustomSet5.fromList(List())
    CustomSet5.member(set, 1) should be(false)
  }

  test("when the element is in the set") {
    val set = CustomSet5.fromList(List(1, 2, 3))
    CustomSet5.member(set, 1) should be(true)
  }

  test("when the element is not in the set") {
    val set = CustomSet5.fromList(List(1, 2, 3))
    CustomSet5.member(set, 4) should be(false)
  }
  ...
  test("empty set is a subset of another empty set") {
    val set1 = CustomSet5.fromList(List())
    val set2 = CustomSet5.fromList(List())
    CustomSet5.isSubsetOf(set1, set2) should be(true)
  }

  test("empty set is a subset of non-empty set") {
    val set1 = CustomSet5.fromList(List())
    val set2 = CustomSet5.fromList(List(1))
    CustomSet5.isSubsetOf(set1, set2) should be(true)
  }

  test("non-empty set is not a subset of empty set") {
    val set1 = CustomSet5.fromList(List(1))
    val set2 = CustomSet5.fromList(List())
    CustomSet5.isSubsetOf(set1, set2) should be(false)
  }

  test("set is a subset of set with exact same elements") {
    val set1 = CustomSet5.fromList(List(1, 2, 3))
    val set2 = CustomSet5.fromList(List(1, 2, 3))
    CustomSet5.isSubsetOf(set1, set2) should be(true)
  }

  test("set is a subset of larger set with same elements") {
    val set1 = CustomSet5.fromList(List(1, 2, 3))
    val set2 = CustomSet5.fromList(List(4, 1, 2, 3))
    CustomSet5.isSubsetOf(set1, set2) should be(true)
  }

  test("set is not a subset of set that does not contain its elements") {
    val set1 = CustomSet5.fromList(List(1, 2, 3))
    val set2 = CustomSet5.fromList(List(4, 1, 3))
    CustomSet5.isSubsetOf(set1, set2) should be(false)
  }
  ...
  test("the empty set is disjoint with itself") {
    val set1 = CustomSet5.fromList(List())
    val set2 = CustomSet5.fromList(List())
    CustomSet5.isDisjointFrom(set1, set2) should be(true)
  }
  ...
  test("sets are not disjoint if they share an element") {
    val set1 = CustomSet5.fromList(List(1, 2))
    val set2 = CustomSet5.fromList(List(2, 3))
    CustomSet5.isDisjointFrom(set1, set2) should be(false)
  }

  test("sets are disjoint if they share no elements") {
    val set1 = CustomSet5.fromList(List(1, 2))
    val set2 = CustomSet5.fromList(List(3, 4))
    CustomSet5.isDisjointFrom(set1, set2) should be(true)
  }
  ...
  test("empty sets are equal") {
    val set1 = CustomSet5.fromList(List())
    val set2 = CustomSet5.fromList(List())
    CustomSet5.isEqual(set1, set2) should be(true)
  }

  test("empty set is not equal to non-empty set") {
    val set1 = CustomSet5.fromList(List())
    val set2 = CustomSet5.fromList(List(1, 2, 3))
    CustomSet5.isEqual(set1, set2) should be(false)
  }
  ...
  test("sets with the same elements are equal") {
    val set1 = CustomSet5.fromList(List(1, 2))
    val set2 = CustomSet5.fromList(List(2, 1))
    CustomSet5.isEqual(set1, set2) should be(true)
  }

  test("sets with different elements are not equal") {
    val set1 = CustomSet5.fromList(List(1, 2, 3))
    val set2 = CustomSet5.fromList(List(1, 2, 4))
    CustomSet5.isEqual(set1, set2) should be(false)
  }

  test("set is not equal to larger set with same elements") {
    val set1 = CustomSet5.fromList(List(1, 2, 3))
    val set2 = CustomSet5.fromList(List(1, 2, 3, 4))
    CustomSet5.isEqual(set1, set2) should be(false)
  }
  ...
  test("add to empty set") {
    val set = CustomSet5.fromList(List())
    val expected = CustomSet5.fromList(List(3))
    CustomSet5.isEqual(CustomSet5.insert(set, 3), expected) should be(true)
  }

  test("add to non-empty set") {
    val set = CustomSet5.fromList(List(1, 2, 4))
    val expected = CustomSet5.fromList(List(1, 2, 3, 4))
    CustomSet5.isEqual(CustomSet5.insert(set, 3), expected) should be(true)
  }

  test("adding an existing element does not change the set") {
    val set = CustomSet5.fromList(List(1, 2, 3))
    val expected = CustomSet5.fromList(List(1, 2, 3))
    CustomSet5.isEqual(CustomSet5.insert(set, 3), expected) should be(true)
  }
  ...
  test("intersection of two empty sets is an empty set") {
    val set1 = CustomSet5.fromList(List())
    val set2 = CustomSet5.fromList(List())
    val expected = CustomSet5.fromList(List())
    CustomSet5.isEqual(CustomSet5.intersection(set1, set2), expected) should be(true)
  }

  test("intersection of an empty set and non-empty set is an empty set") {
    val set1 = CustomSet5.fromList(List())
    val set2 = CustomSet5.fromList(List(3, 2, 5))
    val expected = CustomSet5.fromList(List())
    CustomSet5.isEqual(CustomSet5.intersection(set1, set2), expected) should be(true)
  }

  test("intersection of a non-empty set and an empty set is an empty set") {
    val set1 = CustomSet5.fromList(List(1, 2, 3, 4))
    val set2 = CustomSet5.fromList(List())
    val expected = CustomSet5.fromList(List())
    CustomSet5.isEqual(CustomSet5.intersection(set1, set2), expected) should be(true)
  }
  ...
  test("intersection of two sets with no shared elements is an empty set") {
    val set1 = CustomSet5.fromList(List(1, 2, 3))
    val set2 = CustomSet5.fromList(List(4, 5, 6))
    val expected = CustomSet5.fromList(List())
    CustomSet5.isEqual(CustomSet5.intersection(set1, set2), expected) should be(true)
  }
  ...
  test(
    ...
    val set1 = CustomSet5.fromList(List(1, 2, 3, 4))
    val set2 = CustomSet5.fromList(List(3, 2, 5))
    val expected = CustomSet5.fromList(List(2, 3))
    println(s"intersection. expected: $expected, actual: " + CustomSet5.intersection(set1, set2))
    CustomSet5.isEqual(CustomSet5.intersection(set1, set2), expected) should be(
      true)
  }
  ...
  test("difference of two empty sets is an empty set") {
    ...
    val set2 = CustomSet5.fromList(List())
    val expected = CustomSet5.fromList(List())
    CustomSet5.isEqual(CustomSet5.difference(set1, set2), expected) should be(true)
  }
  ...
  test(
    ...
    val set1 = CustomSet5.fromList(List(3, 2, 1))
    val set2 = CustomSet5.fromList(List(2, 4))
    val expected = CustomSet5.fromList(List(1, 3))
    println(s"difference. expected: $expected, actual: " + CustomSet5.difference(set1, set2))
    CustomSet5.isEqual(CustomSet5.difference(set1, set2), expected) should be(true)
  }
  ...
  test("union of empty sets is an empty set") {
    ...
    val set2 = CustomSet5.fromList(List())
    val expected = CustomSet5.fromList(List())
    CustomSet5.isEqual(CustomSet5.union(set1, set2), expected) should be(true)
  }

  test("union of an empty set and non-empty set is the non-empty set") {
    val set1 = CustomSet5.fromList(List())
    val set2 = CustomSet5.fromList(List(2))
    val expected = CustomSet5.fromList(List(2))
    CustomSet5.isEqual(CustomSet5.union(set1, set2), expected) should be(true)
  }

  test("union of a non-empty set and empty set is the non-empty set") {
    val set1 = CustomSet5.fromList(List(1, 3))
    val set2 = CustomSet5.fromList(List())
    val expected = CustomSet5.fromList(List(1, 3))
    CustomSet5.isEqual(CustomSet5.union(set1, set2), expected) should be(true)
  }
  ...
  test("union of non-empty sets contains all unique elements") {
    val set1 = CustomSet5.fromList(List(1, 3))
    val set2 = CustomSet5.fromList(List(2, 3))
    val expected = CustomSet5.fromList(List(3, 2, 1))
    CustomSet5.isEqual(CustomSet5.union(set1, set2), expected) should be(true)
  }
}
