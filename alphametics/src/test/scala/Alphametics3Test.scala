...
class Alphametics3Test extends FunSuite with Matchers {
  ...
  test("puzzle with ten letters") {
    Alphametics3.solve("AND + A + STRONG + OFFENSE + AS + A + GOOD == DEFENSE") should be(
      Some(
        Map('A' -> 5,
          'D' -> 3,
          'E' -> 4,
