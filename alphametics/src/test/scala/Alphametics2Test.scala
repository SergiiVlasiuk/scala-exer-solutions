...
/** @version 1.3.0 */
class Alphametics2Test extends FunSuite with Matchers {

  test("puzzle with explanation SEND + MORE == MONEY") {
    Alphametics2.solve("SEND + MORE == MONEY") should be(
      Some(Map('E' -> 5, 'N' -> 6, 'Y' -> 2, 'M' -> 1, 'R' -> 8, 'O' -> 0, 'D' -> 7, 'S' -> 9)))
  }

  test("puzzle with three letters") {
    Alphametics2.solve("I + BB == ILL") should be(
      Some(Map('B' -> 9, 'I' -> 1, 'L' -> 0)))
  }

  test("solution must have unique value for each letter") {
    Alphametics2.solve("A == B") should be(None)
  }

  test("leading zero solution is invalid") {
    Alphametics2.solve("ACA + DD == BD") should be(None)
  }

  test("puzzle with two digits final carry") {
    Alphametics2.solve("A + A + A + A + A + A + A + A + A + A + A + B == BCC") should be(
      Some(Map('A' -> 9, 'B' -> 1, 'C' -> 0)))
  }

  test("puzzle with four letters") {
    Alphametics2.solve("AS + A == MOM") should be(
      Some(Map('A' -> 9, 'M' -> 1, 'O' -> 0, 'S' -> 2)))
  }

  test("puzzle with six letters") {
    Alphametics2.solve("NO + NO + TOO == LATE") should be(
      Some(Map('A' -> 0, 'E' -> 2, 'L' -> 1, 'N' -> 7, 'O' -> 4, 'T' -> 9)))
  }

  test("puzzle with seven letters") {
    Alphametics2.solve("HE + SEES + THE == LIGHT") should be(
      Some(
        Map('E' -> 4,
          'G' -> 2,
          'H' -> 5,
  ...
  test("puzzle with ten letters") {
    Alphametics2.solve("AND + A + STRONG + OFFENSE + AS + A + GOOD == DEFENSE") should be(
      Some(
        Map('A' -> 5,
          'D' -> 3,
          'E' -> 4,
