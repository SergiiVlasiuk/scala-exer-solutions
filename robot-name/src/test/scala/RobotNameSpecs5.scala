...
class RobotNameSpecs5 extends FunSpec with Matchers {
  val nameRegex = """[A-Z]{2}\d{3}"""

  it ("has a name") {
    new Robot5().name should fullyMatch regex nameRegex
  }

  it ("does not change its name") {
    val robot = new Robot5
    val name = robot.name
    robot.name should be (name)
  }

  it ("does not have the same name as other robots") {
    new Robot5().name should not be new Robot5().name
  }
  ...
  // The Robot5 code needs to be efficient enough to allow all 676,000 unique names to be generated.
