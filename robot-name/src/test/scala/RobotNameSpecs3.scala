...
class RobotNameSpecs3 extends FunSpec with Matchers {
  val nameRegex = """[A-Z]{2}\d{3}"""

  it ("has a name") {
    new Robot3().name should fullyMatch regex nameRegex
  }

  it ("does not change its name") {
    val robot = new Robot3
    val name = robot.name
    robot.name should be (name)
  }

  it ("does not have the same name as other robots") {
    new Robot3().name should not be new Robot3().name
  }
  ...
  // The Robot3 code needs to be efficient enough to allow all 676,000 unique names to be generated.
