...
class Bank2Account2Test extends FunSuite with Matchers with Conductors with IntegrationPatience {
  test("open account") {
    Bank2.openAccount().getBalance should be(Some(0))
  }

  test("incrementing and checking balance") {
    val acct = Bank2.openAccount()
