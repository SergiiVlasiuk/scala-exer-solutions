...
class Bank3Account3Test extends FunSuite with Matchers with Conductors with IntegrationPatience {
  test("open account") {
    Bank3.openAccount().getBalance should be(Some(0))
  }

  test("incrementing and checking balance") {
    val acct = Bank3.openAccount()
